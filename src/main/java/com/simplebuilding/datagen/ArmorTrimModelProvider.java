...
public class ArmorTrimModelProvider implements DataProvider {
            ...
            for (Armour armour : armour()) {
                ...
                for (Holder.Reference<TrimPattern> pattern : patterns) {
                    ...
                    for (Holder.Reference<TrimMaterial> material : materials) {
                        String colour = McVersion.trimColourSuffix(material.value(), armour.asset());
    ...
    private static JsonObject vanillaTrimSelect(Armour armour) {
        ...
        // (vanilla material key, palette suffix) - the lookup differs between MC 26.2 and 26.3.
        for (Map.Entry<ResourceKey<TrimMaterial>, String> material : ModelGenCompat.vanillaTrimModels().entrySet()) {
            JsonObject entry = new JsonObject();
            entry.add("model", baseModel(armour, modelLocation.withSuffix("_" + material.getValue() + "_trim")));
            entry.addProperty("when", material.getKey().identifier().toString());
            cases.add(entry);
        }
        JsonObject select = new JsonObject();
        select.addProperty("type", "minecraft:select");
