 ...
 * zurückgesetzt wird.
 *
 * <p><b>Diese Datei gilt nur für die 26.2-Linie.</b> Die 1.21.11-Fassung unter
 * {@code mc1_21_11/fabric/} liest die Komponenten direkt über
 * {@code item.components()}; siehe {@link #itemComponents} dazu, warum das hier
 * nicht geht.
 */
public class WikiDataProvider implements DataProvider {
    ...
    public WikiDataProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
        this.output = output;
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        return this.registries.thenCompose(lookup -> write(cache, lookup));
    }

    private CompletableFuture<?> write(CachedOutput cache, HolderLookup.Provider lookup) {
        Map<Identifier, DataComponentMap> components = itemComponents(lookup);
    ...
    /**
     * Ab MC 26.2 hängen die Item-Komponenten nicht mehr am Item selbst: der
     * Konstruktor legt nur einen Initializer in
     * {@code BuiltInRegistries.DATA_COMPONENT_INITIALIZERS} ab, und die fertige
     * Map wird später gegen eine {@link HolderLookup.Provider} gebaut und an den
     * Registry-Holder gebunden. Im Datagen ist diese Bindung noch nicht
     * passiert – {@code item.components()} wirft dort
     * {@code NullPointerException: Components not bound yet}.
     *
     * <p>Deshalb bauen wir die Maps hier selbst und lesen sie über
     * {@code forEach}, statt {@code apply()} aufzurufen: {@code apply()} würde
     * die Holder global binden, und Provider laufen nebenläufig.
     */
    private static Map<Identifier, DataComponentMap> itemComponents(HolderLookup.Provider lookup) {
        Map<Identifier, DataComponentMap> out = new HashMap<>();
        for (DataComponentInitializers.PendingComponents<?> pending
                : BuiltInRegistries.DATA_COMPONENT_INITIALIZERS.build(lookup)) {
            if (Registries.ITEM.equals(pending.key())) {
                collect(pending, out);
            }
        }
        return out;
    ...
    /** Eigene Methode, damit der Wildcard aus {@code PendingComponents<?>} gebunden wird. */
    private static <T> void collect(DataComponentInitializers.PendingComponents<T> pending,
                                    Map<Identifier, DataComponentMap> out) {
        pending.forEach((holder, map) -> out.put(holder.key().identifier(), map));
    }
