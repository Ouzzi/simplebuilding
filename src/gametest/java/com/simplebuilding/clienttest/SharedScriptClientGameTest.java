...
public final class SharedScriptClientGameTest implements FabricClientGameTest {
    ...
    private void runScript(ClientGameTestContext context, TestSingleplayerContext singleplayer,
        ...
        try {
            ...
            // See Harness#releaseAllInput: a failed script skips its own release steps, and what
            // it left held is charged to the next script.
            try {
                harness.releaseAllInput();
            } catch (Throwable cleanup) {
                failures.add(entry.name() + ": could not release the held input afterwards: " + cleanup);
            }
        }
    }
    ...
    private static final class FabricHarness implements Harness {
        ...
        /**
         * What this harness is currently holding down, so a failed script can be undone.
         *
         * <p>Kept here rather than asked of Fabric: {@code TestInputImpl} tracks the same thing in
         * a private static set and does not expose it. Tracking it a second time is cheap and
         * keeps the two drivers saying the same thing.
         */
