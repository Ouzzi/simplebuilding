...
P6_MUTATIONS: list[Mutation] = [
    ...
    Mutation("strip-speed-divisor-two",
             ...
             "case 2 -> 3.0F;",
             "case 2 -> 2.0F;",
             "mining_enchantment_game_test_strip_miner_divides_the_destroy_progress_per_level",
             "Strip Miner 2 left the destroy progress at",
    ...
    Mutation("hammer-charge-clamp",
             ...
             "return Math.clamp(time, RESHAPE_MIN_TICKS, 50);",
    ...
    Mutation("hammer-efficiency-factor",
             ...
             "float factor = materialSpeed + (efficiencyLevel * 4.0f);",
             "sledgehammer_game_test_sledgehammer_charge_time_shortens_with_material_and_efficiency",
    ...
    Mutation("radius-ignores-sneak",
             ...
             "a sneaking hammer takes a single block - no Radius ring and no 3x3",
             kind="server"),
    ...
    Mutation("trim-damage-floor",
             ...
             "the 20% damage floor is gone",
             "no trim set reduces damage below twenty percent",
             kind="server"),
    Mutation("trim-ward-conditional",
             f"{SHARED}/util/TrimEffectUtil.java",
             "        generic += calculateReduction(entity, \"ward\", WARD_ALL, progressMult);",
             "        if (source.is(DamageTypeTags.IS_FIRE)) generic += calculateReduction(entity, \"ward\", WARD_ALL, progressMult);",
             "trim_effect_game_test_damage_reduction_follows_the_pattern_and_keeps_its_floor",
...
P6B_MUTATIONS: list[Mutation] = [
    ...
    Mutation('trim-snout-narrowed',
             ...
             '        if (source.is(DamageTypes.ON_FIRE)) multiplier -= calculateReduction(entity, "snout", SNOUT_FIRE, progressMult);',
    ...
    Mutation('trim-rib-cadence-forty',
             ...
             kind="server"),
    # Seit 2026-09 ist der Laufbonus ein movement_speed-Modifikator (TrimAttributeHandler) statt eines
    # getSpeed-Eingriffs mit Schwimm-/Elytra-Waechter; die Gegenprobe prueft jetzt "nur Spieler".
    Mutation('trim-land-speed-under-elytra',
             'common/src/shared/java/com/simplebuilding/util/TrimAttributeHandler.java',
             '                player ? TrimEffectUtil.getLandSpeedMultiplier(entity) - 1.0f : 0f,',
             '                TrimEffectUtil.getLandSpeedMultiplier(entity) - 1.0f,',
             'trim_wiring_game_test_the_player_mixin_delivers_speed_hunger_and_experience_behind_its_guards',
             'a bolt-trimmed armour stand got the walking speed modifier',
             'the bolt/redstone walking bonus is a player bonus - mobs and armour stands do not get it',
             kind="server"),
    ...
    Mutation('loot-config-guard-dropped',
             ...
             kind="server"),
    Mutation('loot-netherite-core-in-every-bastion-chest',
             'common/src/shared/java/com/simplebuilding/loot/ModLootTableModifications.java',
             '                    .add(item(ModItems.GOLD_CORE, 1))\n                    .add(counted(ModItems.NETHERITE_NUGGET, 12, 1, 4))',
             '                    .add(item(ModItems.GOLD_CORE, 1))\n                    .add(item(ModItems.NETHERITE_CORE, 2))\n                    .add(counted(ModItems.NETHERITE_NUGGET, 12, 1, 4))',
             'config_option_game_test_loot_balance_keeps_every_chest_within_its_budget',
             'which belongs to the treasure room only',
             'the netherite core and the apples sit in the pool only the bastion treasure room gets, not in the one every bastion chest rolls',
             kind="server"),
    Mutation('loot-mansion-empty-weight-starved',
             'common/src/shared/java/com/simplebuilding/loot/ModLootTableModifications.java',
             '                    .add(item(ModItems.QUIVER, 3))\n                    .add(EmptyLootItem.emptyItem().setWeight(30)));',
             '                    .add(item(ModItems.QUIVER, 3))\n                    .add(EmptyLootItem.emptyItem().setWeight(2)));',
             'config_option_game_test_loot_balance_keeps_every_chest_within_its_budget',
             'mod stacks per chest, outside',
             'a woodland mansion chest brings at most 0.7 mod stacks on average; the old weights gave more than two',
             kind="server"),
    ...
    Mutation('dyn-light-update-copy-drops-waterlogged',
             # Seit 2026-09 teilen Erstplatzierung und Aktualisierung einen setBlock (placeOrUpdate); die
             # Mutation laesst die Aktualisierung eines stehenden Lichtblocks das Wasser verlieren.
             ...
             '        boolean isWater = currentState.getFluidState().is(FluidTags.WATER) && !currentState.is(Blocks.LIGHT);',
    ...
    Mutation('hammer-stairs-to-slab-branch',
             ...
             kind="server"),
    # Welle 10 (2026-09-25): the three forward name fallbacks of reshapeTarget, one each. Every one
    # was seen red on both lines before it went in here.
    Mutation('hammer-forward-planks-fallback',
             'common/src/shared/java/com/simplebuilding/items/custom/SledgehammerItem.java',
             '            if (stairs.isEmpty() && id.endsWith("_planks")) stairs = reshapeLookup(namespace, id.substring(0, id.length() - 7) + "_stairs");\n',
             '',
             'sledgehammer_game_test_sledgehammer_reshapes_full_blocks_stairs_and_slabs',
             'the hammer reshapes Block{minecraft:oak_planks} into null',
             'oak_planks becomes oak_stairs (the _planks fallback)',
             kind="server"),
    Mutation('hammer-forward-block-fallback',
             'common/src/shared/java/com/simplebuilding/items/custom/SledgehammerItem.java',
             '            if (stairs.isEmpty() && id.endsWith("_block")) stairs = reshapeLookup(namespace, id.substring(0, id.length() - 6) + "_stairs");\n',
             '',
             'sledgehammer_game_test_sledgehammer_reshapes_full_blocks_stairs_and_slabs',
             'the hammer reshapes Block{minecraft:quartz_block} into null',
             'quartz_block becomes quartz_stairs (the _block fallback)',
             kind="server"),
    Mutation('hammer-forward-plural-fallback',
             'common/src/shared/java/com/simplebuilding/items/custom/SledgehammerItem.java',
             '            if (stairs.isEmpty() && id.endsWith("s")) stairs = reshapeLookup(namespace, id.substring(0, id.length() - 1) + "_stairs");\n',
             '',
             'sledgehammer_game_test_sledgehammer_reshapes_full_blocks_stairs_and_slabs',
             'the hammer reshapes Block{minecraft:bricks} into null',
             'bricks becomes brick_stairs (the plural-s fallback)',
             kind="server"),
]
...
P9_MUTATIONS: list[Mutation] = [
    ...
    # Seit 2026-09 liest der Bildschirm seine Zahlen aus TrimBonusCatalog - der Anker zog mit um.
    Mutation('p9-trim-enderite-row',
             f'{SHARED}/util/TrimBonusCatalog.java',
             'list.add(new Bonus("all_protection", ENDERITE_ALL, Kind.PERCENT));',
             'list.add(new Bonus("all_protection", ENDERITE_ALL * 2, Kind.PERCENT));',
...
P10_MUTATIONS: list[Mutation] = [
    ...
    Mutation('p10-backpack-shift-click-storage-by-index',
             f'{SHARED}/screen/BackpackMenu.java',
             '        storage.sort(Comparator.<Slot>comparingInt(s -> s.y).thenComparingInt(s -> s.x));',
             '',
             _BP + 'shift_click_from_the_hotbar_fills_inventory_and_backpack_as_one_storage',
             'glass in the partly filled backpack slot after a shift-click from the hotbar',
             'shift-click fills the storage in screen order, not slot index order', kind="server"),
    Mutation('p10-backpack-shift-click-backpack-before-inventory',
             f'{SHARED}/screen/BackpackMenu.java',
             '            if (!moveItemStackTo(stack, this.storageSlots)) {',
             '            if (!this.moveItemStackTo(stack, BACKPACK_SLOT_START, this.backpackSlotEnd, false)\n'
             ...
             'shift-click from the hotbar tops up stacks in main inventory and backpack alike', kind="server"),
    Mutation('p10-backpack-shift-click-storage-into-backpack',
             f'{SHARED}/screen/BackpackMenu.java',
             '        } else if (storage) {\n'
             '            if (!this.moveItemStackTo(stack, USE_ROW_SLOT_START, USE_ROW_SLOT_END, false)) {',
             '        } else if (storage) {\n'
             '            if (!moveItemStackTo(stack, this.storageSlots)) {',
             _BP + 'shift_click_sends_storage_to_the_hotbar_and_armor_to_its_slot',
             'shift-click from the main inventory did not go to the hotbar as in vanilla',
             'shift-click from main inventory and backpack goes to the hotbar', kind="server"),
    Mutation('p10-backpack-shift-click-armor-stays',
             f'{SHARED}/screen/BackpackMenu.java',
             '        } else if (equipmentSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR\n',
             '        } else if (false && equipmentSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR\n',
             _BP + 'shift_click_sends_storage_to_the_hotbar_and_armor_to_its_slot',
             'shift-click on a helmet in the backpack did not put it on the empty head slot',
             'armour shift-clicked in the backpack goes onto its empty slot', kind="server"),
    ...
    # --- sledgehammer: slowdown per block and sneaking (owner feedback 2026-09-24) ---------------
    Mutation('p10-hammer-no-slowdown',
             f'{SHARED}/util/SledgehammerUtils.java',
             ...
             'mining progress for a full 3x3 of stone',
             'a full 3x3 mines at a third of the lone-block speed', kind="server"),
    ...
    Mutation('p10-hammer-slowdown-uncapped',
             f'{SHARED}/util/SledgehammerUtils.java',
             'Math.min(blocks, SPEED_BLOCK_CAP)',
             'blocks',
             'sledgehammer_game_test_sledgehammer_speed_and_block_count_scale_with_its_enchantments',
             'mining progress for two full 5x5 layers',
             'the slowdown stops at twenty-five blocks', kind="server"),
    Mutation('p10-hammer-sneak-takes-the-field',
             f'{SHARED}/items/custom/SledgehammerItem.java',
             "        if (player.isShiftKeyDown()) {\n            positions.add(initialPos);\n            return positions;\n        }",
             "        if (false) {\n            positions.add(initialPos);\n            return positions;\n        }",
             'sledgehammer_game_test_sledgehammer_sneaking_breaks_only_the_targeted_block',
             'positions a sneaking swing takes',
             'a sneaking hammer returns the origin only', kind="server"),
    # --- shears on wool ------------------------------------------------------------------------
    Mutation('p10-shears-string-count',
             f'{SHARED}/util/ShearsWoolInteraction.java',
             'new ItemStack(Items.STRING, STRING_PER_WOOL)',
             'new ItemStack(Items.STRING, 3)',
             'tool_behaviour_game_test_shears_turn_placed_wool_into_four_string_and_wear_by_one',
             'string dropped by one sheared wool block',
             'a sheared wool block drops four string', kind="server"),
    Mutation('p10-shears-no-wear',
             f'{SHARED}/util/ShearsWoolInteraction.java',
             '                shears.hurtAndBreak(SHEARS_DAMAGE, player, context.getHand().asEquipmentSlot());',
             '                // no wear',
             'tool_behaviour_game_test_shears_turn_placed_wool_into_four_string_and_wear_by_one',
             'shears wear after shearing one wool block',
...
P11_MUTATIONS: list[Mutation] = [
    ...
    Mutation('p11-detector-rare-classes-common',
             ...
             'the walls each ore is seen through',
             'ancient debris and the End ores carry the weakest signal (5)', kind="server"),
    Mutation('p11-detector-radius-grows-common',
             ...
             _OD + 'all_ores_reach_follows_the_ore_rarity_and_radius_stretches_the_rare_ones',
             'the walls each ore is seen through',
             'Radius lifts the common signal to 22, not more', kind="server"),
    ...
    Mutation('p11-detector-rare-budget-old',
             ...
             _OD + 'all_ores_reach_follows_the_ore_rarity_and_radius_stretches_the_rare_ones',
             'the walls each ore is seen through',
             'diamond and emerald carry a signal of 9', kind="server"),
    # --- ore detector: penetration through material (2026-09-25) ----------------------------------
    Mutation('p11-detector-air-free',
             _ODI,
             '        AIR(0.125),',
             '        AIR(0.0),',
             _OD + 'open_air_reach_ends_at_the_range_of_each_ore_class',
             'ancient debris behind two netherrack and then air',
             'open air costs the signal 0.125 per block', kind="server"),
    Mutation('p11-detector-range-unchecked',
             _ODI,
             '                    if (distanceSq > (double) range * range) continue;',
             '',
             _OD + 'open_air_reach_ends_at_the_range_of_each_ore_class',
             'the farthest block through open air each ore is found at',
             'each ore class ends at its own range, not at the widest scan radius', kind="server"),
    Mutation('p11-detector-end-stone-dense',
             _ODI,
             '        if (state.is(Blocks.END_STONE)) return Material.SOFT;',
             '',
             _OD + 'dense_blocks_shorten_the_beam_more_than_soft_ones',
             'the material step of each sample block',
             'end stone is soft, the host rock of the End ores', kind="server"),
    Mutation('p11-detector-ends-charged',
             _ODI,
             '        for (int i = 1; i < crossings; i++) {',
             '        for (int i = 1; i <= crossings; i++) {',
             _OD + 'dense_blocks_shorten_the_beam_more_than_soft_ones',
             'the loss through netherrack, stone, deepslate, obsidian and air',
             'the target block itself is not charged', kind="server"),
    Mutation('p11-detector-touch-discount',
             _ODI,
             '    public static final double TOUCH_LOSS_FACTOR = 0.5;',
             '    public static final double TOUCH_LOSS_FACTOR = 0.6;',
             _OD + 'constructors_touch_doubles_the_reach_through_solid_rock',
             'how many blocks of rock an iron ore is seen through',
             "Constructor's Touch halves every block's loss", kind="server"),
    ...
    Mutation('p11-bricks-craft-two',
             ...
             'four polished astralit make four bricks', kind="server"),
    ...
    Mutation('p11-ender-quartz-wall-not-a-wall',
             'src/main/generated/data/minecraft/tags/block/walls.json',
             '    "simplebuilding:ender_quartz_brick_wall",\n',
             '',
             _DI + 'end_brick_sets_are_crafted_cut_mined_and_tagged_like_vanilla',
             'the vanilla shape tags the ender_quartz stairs, slab and wall belong to',
             'the ender quartz brick wall connects like a vanilla wall', kind="server"),
    Mutation('p11-polished-cut-missing',
             f'{_GEN}/recipe/nihilith_pillar_from_polished_nihilith_stonecutting.json',
             '"simplebuilding:polished_nihilith"',
             '"simplebuilding:nihilith_block"',
             _DI + 'end_brick_sets_are_crafted_cut_mined_and_tagged_like_vanilla',
             'stonecutter cuts missing',
             'the polished block cuts into the pillar', kind="server"),
    # --- end palettes: recolouring and ender quartz ----------------------------------------------------
    Mutation('p11-recolour-yields-one',
             f'{_GEN}/recipe/astralit_block_from_end_stone.json',
             '"count": 8,',
             '"count": 1,',
             _DI + 'end_palettes_are_recoloured_from_end_stone_and_purpur_like_dye',
             'crafts 1xsimplebuilding:astralit_block',
             'eight end stone around one astralit dust make eight astralit blocks', kind="server"),
    Mutation('p11-recolour-takes-over-coating',
             f'{_GEN}/recipe/polished_ender_quartz_from_purpur_block.json',
             '"M": "simplebuilding:ender_quartz"',
             '"M": "simplebuilding:astralit_dust"',
             _DI + 'end_palettes_are_recoloured_from_end_stone_and_purpur_like_dye',
             'end palette recolouring',
             'the purpur coating grid stays with the astral purpur block', kind="server"),
    Mutation('p11-ender-quartz-yields-one',
             f'{_GEN}/recipe/ender_quartz.json',
             '"count": 2,',
             '"count": 1,',
             _DI + 'ender_quartz_is_crafted_from_astralit_dust_nihilith_shard_and_quartz',
             'crafts 1xsimplebuilding:ender_quartz',
             'one dust, one shard and one quartz make two ender quartz', kind="server"),
    Mutation('p11-ender-quartz-without-quartz',
             f'{_GEN}/recipe/ender_quartz.json',
             '"simplebuilding:nihilith_shard",\n    "minecraft:quartz"',
             '"simplebuilding:nihilith_shard"',
             _DI + 'ender_quartz_is_crafted_from_astralit_dust_nihilith_shard_and_quartz',
             'only AN (A dust, N shard, Q quartz) already crafts',
             'ender quartz costs a quartz as well', kind="server"),
    # --- suspended blocks are solid ---------------------------------------------------------------------
    Mutation('p11-suspended-sand-without-collision',
             f'{SHARED}/blocks/ModBlocks.java',
             'BlockBehaviour.Properties.ofFullCopy(Blocks.SAND).setId(keyOf("suspended_sand"))',
             'BlockBehaviour.Properties.ofFullCopy(Blocks.SAND).noCollision().setId(keyOf("suspended_sand"))',
             'gravity_block_game_test_suspended_sand_and_gravel_hold_items_and_stop_levitating_blocks',
             'should have a full collision box',
             'suspended sand is solid like suspended gravel', kind="server"),
    # --- creative tabs ------------------------------------------------------------------------------
    ...
    # --- Welle 10 (2026-09-25): outstanding counter-checks and the blueprint rough edges -------------
    # Each was seen red before it went in: the crack mutation on client-fabric-262, the server ones on
    # fabric-262 and fabric-12111 (the anchors read the same on both lines).
    Mutation('p11-hammer-upgrade-no-cracks',
             f'{SHARED}/util/SledgehammerProgress.java',
             '        level.destroyBlockProgress(id, pos, crackStage(hits));',
             '        level.destroyBlockProgress(id, pos, -1);',
             'item-rendering', 'SledgehammerProgress (cracks of two blows) did not draw anything',
             'a paused hammer upgrade shows its saved blows as cracks'),
    Mutation('p11-levitating-block-ignores-collision',
             f'{SHARED}/entity/LevitatingBlockEntity.java',
             '        this.move(MoverType.SELF, this.getDeltaMovement());',
             '        this.noPhysics = true;\n        this.move(MoverType.SELF, this.getDeltaMovement());',
             'gravity_block_game_test_suspended_sand_and_gravel_hold_items_and_stop_levitating_blocks',
             'Expected block Levitating Sand',
             'a levitating block lands under suspended sand instead of passing through', kind="server"),
    Mutation('p11-blueprint-counts-one-per-block',
             f'{SHARED}/blueprint/BlueprintMaterials.java',
             '                return Math.max(1, state.getValue(property));',
             '                return 1;',
             'blueprint_game_test_material_list_counts_multi_item_blocks_by_their_state',
             'the material list should count four candles plus one candle as 5',
             'candles, sea pickles, snow layers ... cost as many items as the block holds', kind="server"),
    Mutation('p11-blueprint-builds-one-candle-for-three',
             f'{SHARED}/blueprint/BlueprintMaterials.java',
             '                return Math.max(1, state.getValue(property));',
             '                return 1;',
             'blueprint_game_test_build_consumes_every_candle_and_pickle_it_needs',
             'building should use every item the blocks hold',
             'building a block of three candles uses up three candles', kind="server"),
    Mutation('p11-blueprint-job-not-resumed',
             f'{SHARED}/blueprint/BlueprintBuilder.java',
             '                resumeSaved(serverLevel, player, wandStack);',
             '                // no resume',
             'blueprint_game_test_running_build_survives_logout_and_resumes_with_the_same_blueprint',
             'did not resume the build',
             'a saved build job resumes with the wand and a blueprint of the same code', kind="server"),
    Mutation('p11-blueprint-check-capped',
             f'{SHARED}/blueprint/BlueprintBuilder.java',
             '                if (!check.finished()) {',
             '                if (false) {',
