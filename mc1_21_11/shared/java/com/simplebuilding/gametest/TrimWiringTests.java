...
public final class TrimWiringTests {
    ...
    public static void theCombatFactorWeighsKillsAndDamageByMobCategory(GameTestHelper helper) {
        ...
        Mob hostile = helper.spawnWithNoFreeWill(EntityType.ZOMBIE, new BlockPos(1, 2, 1));
        Mob peaceful = helper.spawnWithNoFreeWill(EntityType.COW, new BlockPos(6, 2, 1));
        Mob ambient = helper.spawnWithNoFreeWill(EntityType.BAT, new BlockPos(1, 2, 6));
        ArmorStand neither = helper.spawn(EntityType.ARMOR_STAND, new BlockPos(6, 2, 6));
    ...
    public static void theTrackerSurvivesTheSaveAndRebasesOnlyOnDeath(GameTestHelper helper) {
        ...
        Assertions.valueEqual(helper, loaderTracker.simplebuilding$getBaseXp(), 108, "BaseXp after loading");
        Assertions.valueEqual(helper, loaderTracker.simplebuilding$getCurrentTime(), 109, "ActiveTicks after loading");
        ...
        Assertions.valueEqual(helper, tracker(helper, legacy).simplebuilding$getBaseTime(), 0,
        ...
        Assertions.valueEqual(helper, travelledTracker.simplebuilding$getBaseXp(), 66,
                "the experience baseline was rebased by a respawn that was not a death");
        Assertions.valueEqual(helper, travelledTracker.simplebuilding$getCurrentTime(), 5678,
        ...
        Assertions.valueEqual(helper, respawnedTracker.simplebuilding$getCurrentTime(), 5678,
                "the active time counter was not carried over into the new life");
        Assertions.valueEqual(helper, respawnedTracker.simplebuilding$getBaseXp(), respawned.totalExperience,
    ...
    /**
     * A {@link ServerPlayer} that is never placed in the level or the player list. It has no
     * {@code connection}, so anything that sends it a packet would throw - and equally it needs no
     * cleanup and cannot collide with another player over a UUID, which is what the save and
     * respawn test needs: it hands out a fresh UUID per call, so the four players of
     * {@link #theTrackerSurvivesTheSaveAndRebasesOnlyOnDeath} stay four distinct players.
     *
     * <p>The 26.2 copy of this file gets the same player from
     * {@code GameTestHelper#makeMockServerPlayer(GameType)}. MC 1.21.11 has no such factory - its
     * helper offers only {@code makeMockPlayer(GameType)}, which returns a plain {@code Player}
     * that carries none of the mod's {@code SurvivalTracerMixin} state, and
     * {@code makeMockServerPlayerInLevel()}, which joins the player list. So the construction 26.2
     * does inside its factory is done here: a {@link ServerPlayer} answering {@code gameMode()},
     * followed by {@code GameType#updatePlayerAbilities} - the same helper
     * {@code ConsumptionAndDurabilityTests#detachedPlayer} uses, repeated because that one is
    ...
    private static SmithingMenu smithingTable(GameTestHelper helper) {
        // The menu never reads the player's position - it is parked on the table only so that the
        // one mock player factory of this file can be reused for it.
        ServerPlayer player = detachedPlayer(helper, new Vec3(1.5, 2.0, 1.5));
    ...
    public static void enderiteTrimTurnsDarkerOnEnderiteArmour(GameTestHelper helper) {
        ...
        String actual = "enderite on enderite " + enderite.assets().assetId(enderiteArmour).suffix()
                + ", enderite on iron " + enderite.assets().assetId(ironArmour).suffix()
                + ", astralit on enderite " + astralit.assets().assetId(enderiteArmour).suffix()
                + ", iron on enderite " + iron.assets().assetId(enderiteArmour).suffix()
                + ", declared enderite on enderite "
                + bootstrapped().get(ModTrimMaterials.ENDERITE).assets().assetId(enderiteArmour).suffix();
        Assertions.valueEqual(helper, actual, "enderite on enderite enderite_darker, enderite on iron enderite, "
        ...
        if (loader.getResource("assets/simplebuilding/textures/palettes/trim/enderite.png") != null) {
            ...
            Assertions.valueEqual(helper, overrides,
