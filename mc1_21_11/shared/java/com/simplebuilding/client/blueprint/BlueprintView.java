...
import com.simplebuilding.mixin.client.GuiGraphicsAccessor;
...
public final class BlueprintView {
    ...
    private static Mesh build(BlueprintModel model) {
        ...
        for (Int2ObjectMap.Entry<BlockState> e : model.blocks().int2ObjectEntrySet()) {
            ...
            BlockStateModel blockModel = mc.getModelManager().getBlockModelShaper().getBlockModel(state);
            parts.clear();
            random.setSeed(42L);
    ...
    private static boolean hasNoGeometry(List<BlockModelPart> parts) {
        for (BlockModelPart part : parts) {
            for (Direction dir : DIRECTIONS) {
    ...
    private static void addQuad(BakedQuad quad, BlockState state, BlockColors blockColors, float ox, float oy, float oz,
        ...
        if (tintIndex != -1) {
            rgb = blockColors.getColor(state, null, null, tintIndex) & 0xFFFFFF;
        }
        float shade = quad.shade() ? shade(n) : 1f;
    ...
    private static int addParticleCube(BlockStateModel blockModel, BlockState state, BlueprintModel model, int bx, int by, int bz,
                                       ...
                                       FloatArrayList pos, FloatArrayList uv, FloatArrayList normals, IntArrayList colors) {
        TextureAtlasSprite sprite = blockModel.particleIcon();
        int added = 0;
        for (Direction dir : DIRECTIONS) {
            BlockState neighbour = model.get(bx + dir.getStepX(), by + dir.getStepY(), bz + dir.getStepZ());
            if (neighbour != null && neighbour.isSolidRender()) {
     ...
     */
    public static void render(GuiGraphics graphics, Mesh mesh, int x, int y, int w, int h, Quaternionf rotation, float zoom) {
        ...
        ((GuiGraphicsAccessor) graphics).simplebuilding$guiRenderState()
                .submitGuiElement(new MeshState(screen, mesh.uv, mesh.color, sorted, pose, texture, area));
    }
