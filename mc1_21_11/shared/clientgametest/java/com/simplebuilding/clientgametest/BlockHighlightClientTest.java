...
public final class BlockHighlightClientTest {
    ...
    private static void octantFillFollowsShowFill(Script script, Later<Path> octantOutlines,
        ...
        octantRemovalReturnsToBaseline(script, baseline, noiseFloor);
    }

    /**
     * The control for the whole octant block: with the octant gone the picture is the stone
     * baseline again.
     *
     * <p>The sledgehammer half has had this control from the start
     * ({@link #itemRemovalReturnsToBaseline}); the octant half had it only in the old NeoForge
     * renderer proof, under the name {@code highlight-f-octant-removed}, and the shared form was
     * written without it. That is a real hole, not a cosmetic one: every octant measurement above
     * compares two pictures taken WITH the octant in hand, so "the picture drifts anyway" explains
     * all of them equally well until something shows the drawing stopping when the cause is taken
     * away.
     *
     * <p>The comparison is against the stone baseline from the very top of the test and its noise
     * floor - not against the octant pictures - because that is the state being returned to. The
     * config is still at opacity 100 with inverted sneak on at this point, which changes nothing
     * with an empty hand: both inputs only steer passes that need a held item.
     *
     * <p>The whole frame, not {@link #RIGHT_HALF_ONLY}: with the octant gone its rangefinder panel
     * has to be gone as well, and restricting the comparison to the right half would let a panel
     * that stayed on screen pass unnoticed.
     */
    private static void octantRemovalReturnsToBaseline(Script script, Later<Path> baseline,
                                                       Later<ScreenshotDiff.Diff> noiseFloor) {
        script.command("clear @a");
        script.awaitPackets();
        script.idle("let the empty hand reach the renderer", 15);

        script.act("control: the octant really is out of the hand", client -> {
            if (client.player == null || !client.player.getMainHandItem().isEmpty()) {
                throw new AssertionError("Control step failed: the octant was not cleared. "
                        + TestScene.describeAim(client));
            }
        });

        Later<Path> removed = script.shot("highlight-k-octant-removed");

        script.verify("removing the octant restores the baseline picture", () -> {
            ScreenshotDiff.Diff residual = ScreenshotDiff.compare(
                    "control (octant removed again)", baseline.get(), removed.get());
            ScreenshotDiff.assertLooksIdentical(noiseFloor.get(), residual,
