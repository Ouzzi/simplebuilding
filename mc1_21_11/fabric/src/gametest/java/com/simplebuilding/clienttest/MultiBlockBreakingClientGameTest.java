...
/**
 * Proves that {@code MultiBlockBreakingSupport} still contributes the extra breaking cracks on the
 * 1.21.11 line. The hook here is {@code WorldRenderEvents.END_EXTRACTION} (1.21.11 has no separate
 * {@code LevelExtractionEvents} class; the event lives on {@code WorldRenderEvents} and hands out a
 * {@code WorldExtractionContext}), and the state list is
 * {@code worldState().blockBreakingRenderStates}.
 *
 ...
 *
 * <p>In 1.21.11 the position is the public field {@code BlockBreakingRenderState.blockPos}
 * (inherited from {@code MovingBlockRenderState}), not the accessor {@code blockPos()} of 26.2.
 *
...
public final class MultiBlockBreakingClientGameTest implements FabricClientGameTest {
    ...
    private Set<BlockPos> mineAndRecord(ClientGameTestContext context, TestSingleplayerContext singleplayer,
        ...
        RendererTestScene.awaitOnClient(context,
                ...
                "the " + itemId + " arriving in the main hand");
    ...
    private static final class BreakingStateRecorder {
        ...
        static void install(ClientGameTestContext context) {
            context.runOnClient(client -> {
                ...
                registered = true;
                WorldRenderEvents.END_EXTRACTION.register(extraction -> {
                    if (!armed) {
                        return;
                    }
                    ...
                    for (BlockBreakingRenderState state : extraction.worldState().blockBreakingRenderStates) {
                        // The field may hold a mutable position that vanilla reuses, so snapshot it.
