...
public final class SimplebuildingNeoForgeClient {
    ...
    private void onClientTick(ClientTickEvent.Post event) {
        ...
        // Rucksack-Taste, gemeinsam mit Fabric.
    ...
    // NeoForge 21.11.45 kennt SubmitCustomGeometryEvent noch nicht (erst ab der 26.x-Linie).
    // Ersatz ist RenderLevelStageEvent.AfterOpaqueBlocks: dieses Sub-Event wird in
    // LevelRenderer#addMainPass direkt nach dem Zeichnen der opaken Chunk-Geometrie gepostet,
    // also VOR submitEntities/submitBlockEntities und vor FeatureRenderDispatcher#renderAllFeatures.
    // Damit landet die eigene Geometrie in derselben Submit-Node-Sammlung wie Entities und wird
    // im selben Flush gezeichnet -- dieselbe Phase, die das Fabric-1.21.11-Modul über
    // WorldRenderEvents.BEFORE_ENTITIES (context.commandQueue()) benutzt.
    // Der SubmitNodeCollector ist in 1.21.11 nur als privates Feld LevelRenderer#submitNodeStorage
    // erreichbar; es wird per META-INF/accesstransformer.cfg geoeffnet.
    // Der PoseStack ist hier null -> RenderLevelStageEvent legt einen identischen
    // (kamerarelativen) PoseStack an, genau wie Vanilla ihn an submitEntities uebergibt.
    private void onRenderLevelStage(RenderLevelStageEvent.AfterOpaqueBlocks event) {
        SubmitNodeCollector collector = event.getLevelRenderer().submitNodeStorage;
