...
Stand 2026-09-25 (HEAD `a6b7917`). Nur Analyse, **am Code ist nichts geändert**.
...
- **Welcher Block verbaut wird**, legt der erste Fund beim Klick fest (`:484`, `:499`). Danach
  sucht jede Position genau diesen Block (`:333`, `:337-357`). Der angeklickte Block spielt keine
  Rolle.
- **Gesetzt wird der Standardzustand** (`:293`, `:320`, `:563` `setBlock`). Es gibt keine
  Ausrichtung und kein `BlockItem.place`. Deshalb greift auch das Nachfüllen durch Berührung des
  Konstrukteurs am Rucksack nicht (siehe 1.4), denn das hängt an `BlockItem#place`
  (`mixin/BlockItemMixin.java:31-52`).

### 1.3 Paarweise Matrix – Baustab, Blaupause, Oktant

| Kombination | Was heute passiert | Beleg |
|---|---|---|
| Stab (Haupthand), Klick auf Block | Baut eine quadratische Fläche vor der Klickseite. Ring für Ring alle 4 Ticks, je Block 1 Haltbarkeit. Durchmesser je Stufe: 3/5/7/9/11/13. Belegte Stellen werden übersprungen, Bau stoppt, wenn Material fehlt. | `BuildingWandItem.java:36-44`, `:551-583`, `:552`, `:557-559`, `:571` |
...
| Stab + **Farbpalette** | Die Vorschau mischt alle gefundenen Blöcke nach Position. Gesetzt wird trotzdem immer der erste Fund (`findFirstBuildingBlock`), bis er leer ist. | `:114-125`, `:328-335` |
| Stab + **Linear** | Verkürzt nur die Ringverzögerung von 4 auf 2 Ticks. | `:43-44`, `:527`, `:578` |
| Stab + **Abdeckung** / **Brücke** | Keine Wirkung. Außerhalb von Registrierung, Beute und Modell gibt es keine Code-Stelle (grep `COVER`/`BRIDGE`). | `ModEnchantments.java:216-243` |
...
| **Oktant + Stab** (beliebige Hände) | **Kein Zusammenspiel.** `BuildingWandItem` liest keinen Oktanten, beide Vorschauen erscheinen einfach nebeneinander. Die im Oktanten gespeicherten Füll-Optionen (hohl, Schicht, Reihenfolge) nutzt niemand. | grep `OctantItem\|OctantShape`: nur Blaupause, Client, Netzwerk. `OctantItem.java:48-56` |
...
Blöcke in ihrer Grundform, also Treppen und Stämme ohne Ausrichtung. Richte sie danach mit Meißel
oder Hammer aus, oder setze solche Blöcke von Hand.
...
- Stab und Oktant: Der Stab füllt keine Oktant-Figur.
- Vorschlaghammer und Oktant: Der Hammer baut keine Oktant-Auswahl ab.
- Die Stab-Verzauberungen *Abdeckung* und *Brücke* wirken nicht, *Linear* macht den Stab nur
  schneller.
