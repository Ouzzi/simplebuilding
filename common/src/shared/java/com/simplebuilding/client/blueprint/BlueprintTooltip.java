...
public final class BlueprintTooltip implements ClientTooltipComponent {
    ...
    public void extractImage(Font font, int x, int y, int w, int h, GuiGraphicsExtractor graphics) {
        ...
        int ty = y + VIEW + 4;
        if (!showMaterials()) {
            graphics.text(font, Component.translatable("simplebuilding.blueprint.tooltip.shift").withStyle(ChatFormatting.DARK_GRAY), x, ty, 0xFFFFFFFF);
            return;
        }
        for (int i = 0; i < rows(); i++) {
            BlueprintMaterials.Entry e = materials.get(i);
            if (e.block() == null) {
                graphics.item(new ItemStack(e.item()), x, ty + i * ROW);
            }
            graphics.text(font, rowText(e), x + 20, ty + i * ROW + 4, 0xFFFFFFFF);
        }
        if (materials.size() > MAX_ROWS) {
            graphics.text(font, Component.translatable("simplebuilding.blueprint.materials.more", materials.size() - MAX_ROWS)
