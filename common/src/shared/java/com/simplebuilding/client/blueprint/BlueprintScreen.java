...
public class BlueprintScreen extends Screen {
    ...
    private void drawHeader(GuiGraphicsExtractor g) {
        ...
        g.text(font, Component.translatable("simplebuilding.blueprint.editor.view"), viewX, labelY, INK_SOFT, false);
    }
    ...
    private void drawMaterials(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < materials.size(); i++) {
            ...
            g.text(font, shown, nameX, ry + 4, INK_SOFT, false);
    ...
    private void drawFooter(GuiGraphicsExtractor g) {
        ...
        if (shown != null) {
            ...
            g.text(font, text, panelX + 8, y, ERROR, false);
        } else if (!parsed.model().isEmpty()) {
            g.text(font, Component.translatable("simplebuilding.blueprint.editor.ok"), panelX + 8, y, OK, false);
        }
        if (!signing) {
    ...
    private void drawSizeIndicator(GuiGraphicsExtractor g, int x, int y, int w) {
        BlueprintModel model = parsed.model();
        ...
        g.text(font, font.plainSubstrByWidth(label.getString(), w), x, y, INK, false);
